...
- 电池前端保存最近 24 小时。
- Android 原生按分钟归档全部监测字段，保留最近 24 小时；回到页面自动回补温度、信号、吞吐及电池历史。
- 历史点保留采集来源：前台、后台、悬浮窗、息屏。图表点按提示和续航记录显示来源，设置页显示每类分钟归档数；旧版本记录显示“未标记”。
- “悬浮窗”表示亮屏后台且悬浮窗显示时由原生服务采集，息屏时优先记为“息屏”；每分钟归档保留首条有效样本的来源。
- 电池趋势显示最近 24 小时，不再显示底部滑块；Android 状态栏与页面顶部栏使用相同背景色。
